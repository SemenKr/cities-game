...
const clickSegmentOption = (label: string) => {
...
describe('Game flows', () => {
    ...
    it('shows a loss when the player timer reaches zero', () => {
        render(<Game />);

        clickSegmentOption('1 мин');
        fireEvent.click(screen.getByRole('button', { name: 'Начать игру' }));
    ...
    it('shows a win when the computer has no city and updates start screen stats after restart', () => {
        render(<Game />);
